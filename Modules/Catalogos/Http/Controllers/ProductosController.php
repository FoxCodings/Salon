...
class ProductosController extends Controller
{
    ...
    {
        $data['proveedores'] = Proveedores::where([['activo',1],['modulo',1]])->get();
        return view('catalogos::productos.create')->with($data);
    }
    ...
    {
        return view('catalogos::productos.show');
    }
    ...
    public function edit($id)
    {
        ...
        $data['codigo'] = $d->getBarcodeHTML(''.$data['productos']->id.'', 'CODABAR');
    ...
    public function tablaproductos(){
      ...
      $registros = Productos::where([['activo', 1],['modulo',1]])->get(); //user es una entidad que se trae desde la app
      ...
      foreach ($data->data as $key => $value) { //el array acciones se constuye en el helpers dropdown - helpers esta con bootsrap
        ...
        //   }else{
            $acciones = [
              "Editar" => [
                "icon" => "edit blue",
                "href" => "/catalogos/productos/$value->id/edit" //esta ruta esta en el archivo web
              ],
              "Eliminar" => [
                "icon" => "edit blue",
                "onclick" => "eliminar($value->id)"
