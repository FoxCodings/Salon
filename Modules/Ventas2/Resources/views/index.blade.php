           ...
           <div class="modal-content">
               ...
               <div class="modal-body">
                 ...
                 <div class="row">
                   <div class="col-md-6">
                       <label for="inputPassword4"  style="font-size:12px;"class="form-label">Pago en Efectivo: </label>
                       <h3> <p id="pago_total_efectivo_ventas2s"></p></h3>
                   </div>
                   <div class="col-md-6">
    ...
    codigo.addEventListener("keydown", evento => {
        ...
        if (evento.keyCode === 13) {
            ...
            $.ajax({
                   ...
                   url:"/ventas2/traerProductos", //si existe colores manda la ruta de anillos el id del usario sino va mandar anillos crear
                   headers: {
...
// var template = radio[0].value;
  ...
  function tablapreventas(){
    ...
    $.ajax({
           ...
           url:"/ventas2/traerPreventas", //si existe colores manda la ruta de anillos el id del usario sino va mandar anillos crear
           headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')//esto siempre debe ir en los ajax
           },
            data: {
  ...
  function monto_totals(){
    ...
    $('#cambio_efectivo_venta2').html('<strong>$ '+total.toFixed(2)+'</strong>')
    $('#cambio_efectivo_ventas2').val(total.toFixed(2));
    $('#pago_total_efectivo_venta2s').html('<strong>$ '+monto+'.00</strong>')
    $('#pago_total_efectivo_ventas2s').html('<strong>$ '+monto+'.00</strong>')
...
$(function() {
  tabla = $('#example_list_art2').dataTable({
    ajax: {
      url: "/ventas2/tablaProductosVenta",
    },
    processing: true,
    serverSide: true,
    stateSave: true,
