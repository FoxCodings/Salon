...
use \Modules\Promociones2\Entities\Promociones;
use \Modules\Promociones2\Entities\Descuentos;
use \Modules\Promociones2\Entities\Cliente_Cumpleano;
use \Modules\Agenda2\Entities\Eventos;
use \Modules\Ventas2\Entities\Ventas;
...
use \Modules\Ventas2\Entities\Certificado;
  ...
  public function __construct()
  {
    ...
    public function index()
    {
      ...
      $fecho = date("Y-m-t", strtotime($date));
        ...
        Cliente_Cumpleano::where([['activo',1],['modulo',2]])->delete();
        ...
        $cliente =  Clientes::where([['activo',1],['modulo',2],['id',$value['cliente']]])->get();
        ...
        (SELECT * FROM cat_alaciado WHERE activo = 1 )
         ...
         //dd($result);
      $data['clientes_cumpleanos'] = Cliente_Cumpleano::where([['activo',1],['modulo',2]])->get();
      ...
      $data['id_cotizadores'] = Citas_diarias::where([
        ...
        return view('ventas2::index')->with($data);
    }
    ...
    {
        return view('ventas2::ver_ventas');
    }
    ...
    public function show_venta($id)
    {
        ...
        return view('ventas2::show_venta')->with($data);
    }
    ...
    {
        return view('ventas2::edit');
    }

    public function preventas()
    {
        return view('ventas2::preventas');
    }
    ...
    public function tablaventas()
    {
      ...
      $registros = Ventas::select(
        ...
        )->join('t_cat_ventas','t_cat_ventas.id_venta','t_ventas.id')->where([['t_ventas.activo', 1],['t_ventas.modulo',2]])->orderby('id','DESC')->get(); //user es una entidad que se trae desde la app
      ...
      foreach ($data->data as $key => $value) { //el array acciones se constuye en el helpers dropdown - helpers esta con bootsrap
        ...
        //   }else{
            $acciones = [
              "Ver Venta" => [
                "icon" => "edit blue",
                "href" => "/ventas2/$value->id/show_venta" //esta ruta esta en el archivo web
              ],
              // "Eliminar" => [
              //   "icon" => "edit blue",
              //   "onclick" => "eliminar($value->id)"
    ...
    public function tablaliquidaciones(){
      ...
      SELECT
        ...
        SUM(t_liquidar.importe) AS total_importe,
