...
class CertificadosController extends Controller
{
    ...
    {
        return view('ventas2::certificados.index');
    }
    ...
    public function create()
    {
        ...
        return view('ventas2::certificados.create')->with($data);
    }
    ...
    {
        return view('ventas2::certificados.show');
    }
    ...
    public function tablacertificados(){
      ...
      $registros = Certificado::where([['activo', 1],['modulo',2]])->get(); 
      ...
      foreach ($data->data as $key => $value) { //el array acciones se constuye en el helpers dropdown - helpers esta con bootsrap
        ...
        if ($value->status == 'Sin Pagar') {
          $acciones = [
            "Editar" => [
              "icon" => "edit blue",
              "href" => "/ventas2/certificados/$value->id/edit" //esta ruta esta en el archivo web
            ],
            "Pagar Efectivo" => [
              "icon" => "edit blue",
          ...
          $acciones = [
            "Editar" => [
              "icon" => "edit blue",
              "href" => "/ventas2/certificados/$value->id/edit",
              "target" => "_blank"
            ],
            "Imprimir Frontal" => [
              "icon" => "edit blue",
              "href" => "/ventas2/certificados/$value->id/imprimir",
              "target" => "_blank"
            ],
            "Imprimir Posterior" => [
              "icon" => "edit blue",
              "href" => "/ventas2/certificados/$value->id/imprimirposterior",
              "target" => "_blank"
            ],
    ...
    $pdf = PDF::loadView('ventas2::certificados.certificado_imprimir', $data);
    ...
    $namePdf = 'Frontal SPA VIT & Live.pdf';
    return $pdf->download($namePdf);
    return $pdf->stream();
    //return view('ventas2::certificados.certificado_imprimir')->with($data);
  }
  ...
  $pdf = PDF::loadView('ventas2::certificados.certificado_posterior_imprimir', $data);
  $pdf->setPaper(array(0,0,612.00, 790.00), 'portrait');
  $pdf->setOptions(['enable_php' => true,'isHtml5ParserEnabled' => true,'isRemoteEnabled' => true,'defaultFont' => 'pacifico_normal_9e96d9e23ad102bc05be47680b365314']);

  $pdf->output();

  $namePdf = 'Posterior SPA VIT & Live.pdf';
  return $pdf->download($namePdf);
  return $pdf->stream();
  ...
  // return $pdf->download($namePdf);
  // return $pdf->stream();
  //return view('ventas2::certificados.certificado_posterior_imprimir')->with($data);
}
