...
if (! function_exists('obtenerModulosActivos2')) {
  function obtenerModulosActivos2() {
    $modulos = Module::all();
    $tmp = [];
    foreach ($modulos as $key => $value) {

      if($value->get('grupo') == 2 || $value->get('grupo') == 3){
        $tmp[( $titulo = $value->get('titulo') ) ? $titulo : $value->get('name')] = $value;
        ksort($tmp);
      }else{
        $tmp;
...
if (! function_exists('obtenerModulo2')) {
  function obtenerModulo2() {
